...
void BallSystem::checkInfections()
{
  for (int i=0; i<balls.size(); i++) {
    for (int j=0; j<balls.size(); j++) {
        if (i!=j){
            if (balls[i].health<255 || balls[j].health<255){
                float b2b_distance = ofDist(balls[i].location.x, balls[i].location.y, balls[j].location.x, balls[j].location.y);
                float sumRad = balls[i].radius + balls[j].radius;

                if (b2b_distance < sumRad){
                    if (balls[i].health < 255 && balls[j].health==255) balls[j].health = 0;
                    else if (balls[j].health < 255 && balls[i].health==255) balls[i].health = 0;
                }
            }
        }
    }
